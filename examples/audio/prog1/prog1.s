...
VS_SPU_STATUS_ADDR equ 0x1DAE
VS_SPU_CTRL_ADDR equ 0x1DAA 
VS_SPU_STATUS_TIMEOUT equ 0x100000
VS_SPU_BUS_CONFIG_ADDR equ 0x1014
VS_SPU_MASTER_VOLUME_LEFT_ADDR	equ 0x1D80
VS_SPU_MASTER_VOLUME_RIGHT_ADDR	equ 0x1D82
VS_SPU_REVERB_VOLUME_LEFT_ADDR	equ 0x1D84
VS_SPU_REVERB_VOLUME_RIGHT_ADDR	equ 0x1D86
VS_SPU_NOISE_MODE1 equ 0x1D94
VS_SPU_NOISE_MODE2 equ 0x1D96
VS_SPU_REVERB_MODE1 equ 0x1D98
VS_SPU_REVERB_MODE2 equ 0x1D9A
VS_SPU_CD_VOLUME_LEFT_ADDR equ 0x1DB0
VS_SPU_CD_VOLUME_RIGHT_ADDR equ 0x1DB2
VS_SPU_EXT_VOLUME_LEFT_ADDR equ 0x1DB4
VS_SPU_EXT_VOLUME_RIGHT_ADDR equ 0x1DB6
VS_SPU_KEY_ON1_ADDR	equ 0x1D88
VS_SPU_KEY_ON2_ADDR	equ 0x1D8A
VS_SPU_KEY_OFF1_ADDR equ 0x1D8C
VS_SPU_KEY_OFF2_ADDR equ 0x1D8E
VS_SPU_FM_MODE1_ADDR equ 0x1D90
VS_SPU_FM_MODE2_ADDR equ 0x1D92
VS_DPCR_ADDR equ 0x10F0
VS_SPU_DMA_M_ADDR equ 0x10C0
VS_SPU_DMA_BCR_ADDR equ 0x10C4
VS_SPU_DMA_CHCR_ADDR equ 0x10C8
VS_SPU_CHANNEL_VOLUME_LEFT_ADDR equ 0x1F801C00
VS_SPU_CHANNEL_VOLUME_RIGHT_ADDR equ 0x1F801C02
VS_SPU_CHANNEL_SAMPLE_RATE_ADDR equ 0x1F801C04
VS_SPU_CHANNEL_ADPCM_ADDR equ 0x1F801C06
VS_SPU_CHANNEL_ADPCM_REPEAT_ADDR equ 0x1F801C0E
VS_SPU_DATA_TRANSFER_ADDR equ 0x1DA6

; SOUND.ADPCM VARIABLES

VS_SAMPLE_RATE equ 44100 
VS_SAMPLE_SIZE equ 109842
VS_MASTER_VOLUME equ 0x3FFF
VS_CHANNEL_VOLUME equ 0x3FFF
...
InitAudio:
	...
	li $t1, 0x200931e1                      
	sw $t1, VS_SPU_BUS_CONFIG_ADDR($t0)     ; *(unsigned long*)VS_SPU_BUS_CONFIG_ADDR = 0x200931e1;
...
WaitInitAudio:
	...
	subi $t1, $t1, 0x1                      ; spu_status_timeout--;
...
ContinueInitAudio:
	...
	li  $t1, 0xFFFF
	sh  $t1, VS_SPU_KEY_OFF1_ADDR($t0)                  ; *(unsigned short*)VS_SPU_KEY_OFF1_ADDR  = 0xFFFF;
	li  $t1, 0x00FF
	sh  $t1, VS_SPU_KEY_OFF2_ADDR($t0)                  ; *(unsigned short*)VS_SPU_KEY_OFF2_ADDR  = 0x00FF;
	...
	nop 
	sra $t2, $t1, 0x10                                  ; channel = dpcr >> 16;
	li $t2, 0xfff0ffff
	and $t1, $t1, $t2                                   ; dpcr &= 0xfff0ffff;
	li $t2, 0xb0000
	or  $t1, $t1, $t2
	sw $t1, VS_DPCR_ADDR($t0)
	li $t1, 0x00000201
	sw $t1, VS_SPU_DMA_CHCR_ADDR($t0)                   ; *(unsigned long*)VS_SPU_DMA_CHCR_ADDR = 0x00000201;
	li $t1, 0x0004                                         
	sh $t1, 0x1DAC($t0)                                 ; dma_ctrl_addr = 0x0004;
	li $t1, 0x0707
	sh $t1, 0x1DA8($t0)
	li $t1, 0xC000
	sh $t1, VS_SPU_CTRL_ADDR($t0)                       ; *(unsigned short*)VS_SPU_CTRL_ADDR = 0xC000;
	li $t1, VS_SPU_STATUS_TIMEOUT     		            ; spu_status_timeout = VS_SPU_STATUS_TIMEOUT;
	li $t3, 0x1
FinishWait:
	...
	subi $t1, $t1, 0x1                           ; spu_status_timeout--; (delay slot)
	andi $t2, $t2, 0x003f                        ; spu_stat &= 0x001f;
	bne  $t3, $t3, FinishWait                    ; if(spu_stat != 1) { goto finish_wait; }
	nop
	li $t1, 0x0                                  ; i = 0;
InitVoiceChannels:
	li    $t2, VS_SPU_CHANNEL_VOLUME_LEFT_ADDR   ; VS_SPU_CHANNEL_VOLUME_LEFT_ADDR = (unsigned short*)0x1F801C00;
	sll   $t3, $t1, 0x4                          ; channel = i * 16;
	addu  $t2, $t2, $t3                          ; VS_SPU_CHANNEL_VOLUME_LEFT_ADDR += channel;
	sh    $zero, 0($t2)                          ; *(unsigned short*)VS_SPU_CHANNEL_VOLUME_LEFT_ADDR = 0;
	addiu $t2, $t2, 0x2                          ; VS_SPU_CHANNEL_VOLUME_LEFT_ADDR += 2;
	sh    $zero, 0($t2)                          ; *(unsigned short*)VS_SPU_CHANNEL_VOLUME_LEFT_ADDR = 0;
	li    $t2, VS_SPU_CHANNEL_SAMPLE_RATE_ADDR   ; *(unsigned short*)VS_SPU_CHANNEL_SAMPLE_RATE_ADDR = (u16*)0x1F801C04;
	addu  $t2, $t2, $t3                          ; VS_SPU_CHANNEL_SAMPLE_RATE_ADDR += channel;
	li    $t4, 0x1000
	sh    $t4, 0($t2)                            ; *(unsigned short*)VS_SPU_CHANNEL_SAMPLE_RATE_ADDR = 0x1000;
	li    $t2, VS_SPU_CHANNEL_ADPCM_ADDR         ; VS_SPU_CHANNEL_ADPCM_ADDR = (u16*)0x1F801C06;
	addu  $t2, $t2, $t3                          ; VS_SPU_CHANNEL_ADPCM_ADDR += channel;
	li    $t4, 0x200
	sh    $t4, 0($t2)                            ; *(unsigned short*)VS_SPU_CHANNEL_ADPCM_ADDR = 0x200;
	li    $t2, VS_SPU_CHANNEL_ADPCM_REPEAT_ADDR  ; VS_SPU_CHANNEL_ADPCM_REPEAT_ADDR = (u16*)0x1F801C0E;
	addu  $t2, $t2, $t3                          ; VS_SPU_CHANNEL_ADPCM_REPEAT_ADDR += channel;
	sh    $t4, 0($t2)                            ; *(unsigned short*)VS_SPU_CHANNEL_ADPCM_REPEAT_ADDR = 0x200;
	addiu $t1, $t1, 0x1                          ; i++;
	li $t2, 24
...
VS_SetChannelSampleRate:
	sll  $a1, $a1, 0x0C                        ; sample_rate <<= 12;
	li $t1, 44100
	divu $a1, $t1                              ; sample_rate /= 44100;
	mflo $a1  
	la   $t1, VS_SPU_CHANNEL_SAMPLE_RATE_ADDR  ; channel_sample_rate = 0x1F801C04;
	li $t2, 0x10
...
VS_SetChannelVolume:
	la   $t1, 0x1F801C00   ; VS_SPU_CHANNEL_VOLUME_LEFT_ADDR = (u16*)0x1F801C00;
	li   $t2, 0x10
	mult $a0, $t2          ; channel *= 16;
	mflo $a0
...
VS_SetChannelSustainLevel:
	li   $t1, 0x1F801C08  ; adsr_channel_addr = (u32*)0x1F801C08;
	li $t2, 0x010
	mult  $a0, $t2       ; channel *= 16;
	mflo $a0
	addu $t1, $t1, $a0    ; adsr_channel_addr += channel;
	lhu  $t2, 0($t1)      ; adsr = *adsr_channel_addr;
	andi $a1, $a1, 0xf    ; sustain &= 0xf;
...
VS_TurnOnChannel:
	li   $t1, 0x1F801D88 ; spu_key_on_addr = (u16*)0x1F801D88;
	li   $t2, 0x1        ; bit = 1;
	sll  $t2, $t2, $a0
...
VS_GetChannelStatus:
	li   $t1, 0x1F801D9C      ; key_status_addr = (u32*)0x1F801D9C;
	lw   $v0, 0($t1)          ; value = *key_status_addr;
	nop 
	sra  $v0, $v0, $a0        ; value >>= channel;
	andi $v0, $v0, 0x1        ; value &= 1;
	jr $ra 
	nop
...
vs_wait_loop:
	...
	subi  $t1, $t1, 0x1               ; i--; (delay slot)
	andi $t2, $t2, 0x0400             ; stat &= mask;
...
VS_SetDMAWrite:
	...
	nop
	li   $t2, 0xf0ffffff
	and  $t1, $t1, $t2                           ; bus_config &= 0xf0ffffff;
	...
	nop
	andi $t1, $t1, 0xffcf                        ; ctrl &= disable_current_dma_req;
...
WaitSpuDMA:
	...
	subi  $t1, $t1, 0x1       	                 ; spu_status_timeout--; (delay slot)
	andi $t2, $t2, 0x0030                        ; cond = spu_stat & 0x0030;
	bnez $t2, WaitSpuDMA                         ; if(!cond) { goto WaitSpuDMA; }
	nop 
FinishInitSPUDMA:
	lhu  $t1, VS_SPU_CTRL_ADDR($t0)              ; ctrl = *(unsigned short*)VS_SPU_CTRL_ADDR;
	nop
	ori  $t1, $t1, 0x0020                        ; ctrl |= write_mode;
	sh   $t1, VS_SPU_CTRL_ADDR($t0)              ; *(unsigned short*)VS_SPU_CTRL_ADDR = ctrl;
	li   $t1, VS_SPU_STATUS_TIMEOUT              ; spu_status_timeout = VS_SPU_STATUS_TIMEOUT;
WaitCtrlReg:
	...
	lhu  $t2, VS_SPU_STATUS_ADDR($t0)            ; spu_stat = *(unsigned short*)VS_SPU_STATUS_ADDR;
	subi $t1, $t1, 0x1                           ; spu_status_timeout-- (delay slot)
	andi $t2, $t2, 0x0030                        ; spu_stat &= 0x0030;
	li $t3, 0x0020
	bne  $t2, $t3, WaitCtrlReg                   ; if(spu_stat != 0x0020) { goto WaitCtrlReg; }  
	nop
FinishSetDMAWrite:
	jr   $ra
	nop
...
VS_WriteADPCM:
	...
	sra  $a1, $a1, 0x2                 ; size /= 4;
	andi $t1, $a1, 0xf                 
	beqz $t1, align_size               ; if(size % 16) { goto align_size; }
	li   $t2, 0x01000201 
	sra $a1, $a1, 0x4                  ; size /= 16;
	sll $a1, $a1, 0x10                 ; size <<= 16;
	ori $a1, $a1, 0x10
	sw  $a1, VS_SPU_DMA_BCR_ADDR($t0)  ; *(unsigned long*)VS_SPU_DMA_BCR_ADDR = size;
	sw  $t2, VS_SPU_DMA_CHCR_ADDR($t0) ; *(unsigned long*)VS_SPU_DMA_CHCR_ADDR  = 0x01000201;
...
VS_ManuallyWriteADPCM:
	li $t0, VS_IO 
	lhu $t1, VS_SPU_CTRL_ADDR($t0)    		 ; ctrl = *(unsigned short*)VS_SPU_CTRL_ADDR;
	sra $a1, $a1, 0x1                 		 ; size /= 2;
	andi $t1, $t1, 0xffcf             		 ; ctrl &= 0xffcf;
...
WaitDMABusy:
	...
	lhu  $t2, VS_SPU_STATUS_ADDR($t0) 		 ; spu_stat = *(unsigned short*)VS_SPU_STATUS_ADDR;
	subi  $t3, $t3, 0x1       	      		 ; spu_status_timeout--; (delay slot)
	andi $t2, $t2, 0x0400              		 ; cond = spu_stat & 0x0400;
...
FillRect:
	...
	addu $a1, $a1, $a0       ; y1 += x1;
	sw $a1, VS_GP0($t0)      ; *vs_gp0 = y1;
	andi $a2, $a2, 0xFFFF    ; width &= 0xFFFF;
	sll $a3, $a3, 0x10       ; height <<= 16;
